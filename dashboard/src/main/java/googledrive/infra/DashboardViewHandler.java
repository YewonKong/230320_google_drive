...
public class DashboardViewHandler {
    ...
    public void whenFileUploaded_then_CREATE_1(
        ...
        try {
            ...
            dashboard.setSize(Integer.valueOf(fileUploaded.getSize()));
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenVideoPrecessed_then_UPDATE_2(
        @Payload VideoPrecessed videoPrecessed
    ) {
        try {
            if (!videoPrecessed.validate()) return;
            // view 객체 조회
            Optional<Dashboard> dashboardOptional = dashboardRepository.findById(
                videoPrecessed.getFileid()
            );

            if (dashboardOptional.isPresent()) {
                Dashboard dashboard = dashboardOptional.get();
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                dashboard.setUrl(videoPrecessed.getUrl());
                // view 레파지 토리에 save
                dashboardRepository.save(dashboard);
            }
        } catch (Exception e) {
